...
namespace DialogueEx {
    ...
    TESTopicInfo* GetNPCInfo(BGSSceneActionPlayerDialogue* playerDialogue, int optionID)
    {
        ...
        for (TESTopicInfo* info : npcInfos) {
            ...
            if (info->infoFlags & TESTopicInfo::kFlag_Random) {
                ...
                if (EvaluateInfoConditions(info, playerDialogue, true)) {
                    randomTopics.push_back(infoCount);
                    randomsFound += 1;

                    if (info->infoFlags & TESTopicInfo::kFlag_RandomEnd) {
                        return npcInfos[randomTopics[rand() % randomsFound]];
                    }
                }
        ...
        _MESSAGE("All the infos failed their condition checks..");
